...
nig.haplo <- read_tsv("../input/step1-male-haplotype-20210602.txt")
dir.info
head(nig.haplo)
```
...
```{r}
# nigra
write_tsv(nig.haplo.new, "../output/Pnigra_haplotype_20220929.txt")
```
...
del.haplo1 <- del.haplo %>% 
  filter(Chrom == "Chr01") %>%
  filter((Pos <= 12100252) | (Pos >= 12667849 & Pos <= 21680968) | (Pos >= 25506608 & Pos <= 41070247) | (Pos >= 48329818)) %>%
  mutate(F1new = F2) %>%
  mutate(F2new = F1) %>%
...
del.haplo8 <- del.haplo %>% 
  filter(Chrom == "Chr08") %>%
  filter((Pos <= 1914708) | (Pos >= 5003493)) %>%
  mutate(F1new = F2) %>%
  mutate(F2new = F1) %>%
  select(Chrom, Pos, M1, M2, F1new, F2new)
...
del.haplo4 <- del.haplo %>% 
  filter(Chrom == "Chr04") %>%
  filter((Pos >= 2791762 & Pos <= 3125774) | (Pos >= 7291672 & Pos <= 19311643)) %>%
  mutate(F1new = F2) %>%
  mutate(F2new = F1) %>%
  select(Chrom, Pos, M1, M2, F1new, F2new)
...
```{r}
# deltoides
write_tsv(del.haplo.new, "../output/Pdeltoides_haplotype_20220930.txt")
```