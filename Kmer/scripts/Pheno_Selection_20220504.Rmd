...
```{r}
pheno <- pheno %>%
  arrange(desc(Blup_Transfo_time_serie_color_y1_y2_y3)) %>%
  select(Old.Name,Blup_Transfo_time_serie_color_y1_y2_y3, everything())
pheno
```
**vessel**
```{r}
...
bulk1 <- pheno1 %>%
  ...
  select(Old.Name, Blup_Transfo_time_serie_color_y1_y2_y3)
bulk1
```

```{r}
pheno2 <- pheno %>%
  arrange(Blup_Transfo_time_serie_color_y1_y2_y3) 
...
Bulk <- full_join(bulk1, bulk2, by=c("Old.Name","Blup_Transfo_time_serie_color_y1_y2_y3","Group"))
Bulk
```
**vessel**
```{r}
...
with(Bulk, shapiro.test(Blup_Transfo_time_serie_color_y1_y2_y3[Group == "1"]))
# Shapiro-Wilk normality test for Bulk2
with(Bulk, shapiro.test(Blup_Transfo_time_serie_color_y1_y2_y3[Group == "2"])) 
```
#### Do the two populations have the same variances? ####
```{r}
res.ftest <- var.test(Blup_Transfo_time_serie_color_y1_y2_y3 ~ Group, data = Bulk)
res.ftest
```

#### Compute unpaired two-samples t-test ####
```{r}
res <- t.test(Blup_Transfo_time_serie_color_y1_y2_y3 ~ Group, data = Bulk, var.equal = TRUE)
res
```
