...
#### Permutation test on t value ####
```{r}
for (j in 2873:length(leaf.Del.Nig.Dos)) {
  Pheno <- leaf.Del.Nig.Dos[,j]
  pheno.name <- colnames(leaf.Del.Nig.Dos)[j]
  geno <- seq(2,2872,3)
  model.tvalue <- tibble(Marker = c("Test"), 
                        Intercept = c(123), 
                        Deltoides = c(123), 
                        Nigra = c(123), 
                        Dosage = c(123))
  ...
  for (i in geno) {
    colnames(leaf.Del.Nig.Dos)
    m <- summary(lm(Pheno ~ leaf.Del.Nig.Dos[,i] + leaf.Del.Nig.Dos[,i+1] + leaf.Del.Nig.Dos[,i+2], data = leaf.Del.Nig.Dos))
    tvalue <- m$coefficients[,3]
    if (length(tvalue) < 4) {
      diff <- 4 - length(tvalue)
      for (j in 1:diff) {
        tvalue <- c(tvalue,"NA")
      }
    }
    T.value <- c(colnames(deltoides_gt)[(i+1)/3+1],tvalue)
    model.tvalue <- rbind(model.tvalue,T.value)
  }
  model.tvalue <- model.tvalue %>% 
    filter(Marker != "Test") %>%
    mutate(CHR = as.numeric(str_replace(string = Marker, pattern = "(...)(..)_(.*)_(.*)",replacement = "\\2"))) %>%
    mutate(BP = as.numeric(str_replace(string = Marker, pattern = "(...)(..)_(.*)_(.*)", replacement = "\\3")))
  
  # save tvalue file
  write_csv(model.tvalue, paste("../Tvalue_Leaf_DND/", pheno.name, ".csv", sep = ""))
}
```
...
### marker check ###
```{r}
test.del <- read_tsv("../F1_450/genotype/Pdeltoides_F1_whole_genome_geno_20221115.txt")
test.nig <- read_tsv("../F1_450/genotype/Pnigra_F1_whole_genome_geno_20221115.txt")
