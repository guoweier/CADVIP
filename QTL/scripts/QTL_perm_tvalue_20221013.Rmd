...
```{r}
leaf.rna <- as.data.frame(left_join(nigra_gt, leaf.new, by = "Old.Name")[c(1,959:1014)])
leaf.rna
```
...
```{r}
for (j in 2:57) {
  observed.t <- qtlscan(leaf.rna[,j], deltoides_gt[,-1], nigra_gt[,-1], dosage_gt[,-1])
  name <- str_replace(string = colnames(leaf.rna)[j], pattern = "Blup_Transfo_(.*)", replacement = "\\1")
  write_csv(observed.t, paste("../perm/DND/observed/Obs_", name, ".csv", sep = ""))
}
...
for (j in 2:57) {
  ...
  perm.results <- sapply(1:n.perm, function(p) {
    ...
    # apply qtlscan
    t <- qtlscan(sample(leaf.rna[,j]), deltoides_gt[,-1], nigra_gt[,-1], dosage_gt[,-1]) # "sample" does permutation, resorting the phenotypes at each permutation.
    apply(t, 2, max, na.rm = TRUE)
    })
  perm.df <- as.data.frame(perm.results)
  name <- str_replace(string = colnames(leaf.rna)[j], pattern = "Blup_Transfo_(.*)", replacement = "\\1")
  write_csv(perm.df, paste("../perm/DND/permutation/Perm_", name, ".csv", sep = ""))
}
...
```{r}
p.files <- tibble(filename = list.files("../perm/Dos/permutation/"))
p.files.paths <- p.files %>%
  mutate(filepath = paste("../perm/Dos/permutation/", filename, sep = "")) %>%
...
```{r}
# apply function for each imported permutation results, and collect them
Perm90s <- tibble(Dos = c(123))
Perm95s <- tibble(Dos = c(123))
Perm99s <- tibble(Dos = c(123))
Name <- c()
...
for (i in 1:length(p.files.paths$filename)) {
  obs.path <- paste("../perm/Dos/observed/Obs_", p.files.paths$phenotype[i], ".csv", sep = "")
  obs.t <- read_csv(obs.path) %>%
    as.data.frame()
  obs.tvals <- cbind(Markers, obs.t)
  colnames(obs.tvals)[1] <- "Marker"
  colnames(obs.tvals)[2] <- "Dos"
  # compare del
  #sign.del90 <- perm.compareone(obs.tvals, 90, 2, i, Name[i])
  #sign.del95 <- perm.compareone(obs.tvals, 95, 2, i, Name[i])
  #sign.del99 <- perm.compareone(obs.tvals, 99, 2, i, Name[i])
  #Sign.Dels <- rbind(Sign.Dels, sign.del90, sign.del95, sign.del99)
  # compare nig
  #sign.nig90 <- perm.compare(obs.tvals, 90, 2, i, Name[i])
  #sign.nig95 <- perm.compare(obs.tvals, 95, 2, i, Name[i])
  #sign.nig99 <- perm.compare(obs.tvals, 99, 2, i, Name[i])
  #Sign.Nigs <- rbind(Sign.Nigs, sign.nig90, sign.nig95, sign.nig99)
  # compare dos
  sign.dos90 <- perm.compare(obs.tvals, 90, 2, i, Name[i])
  sign.dos95 <- perm.compare(obs.tvals, 95, 2, i, Name[i])
  sign.dos99 <- perm.compare(obs.tvals, 99, 2, i, Name[i])
...
#### save results ####
```{r}
write_csv(Sign.Dels, "../perm/Del/Significant/Sign_Dels_20221027.csv")
write_csv(Sign.Nigs, "../perm/Nig/Significant/Sign_Nigs_20221101.csv")
write_csv(Sign.Doss, "../perm/Dos/Significant/Sign_Doss_20221103.csv")
```
